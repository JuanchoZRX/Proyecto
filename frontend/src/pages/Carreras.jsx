...
export default function Carreras() {
  ...
  const filtered = carreras.filter((c) =>
      ...
      circuitoNombre(c.circuitoId).toLowerCase().includes(search.toLowerCase())
  );
      ...
      <Layout>
        <div className="page-header">
          <div>
            ...
            <p className="page-subtitle">
              {filtered.length} de {carreras.length} carrera{carreras.length !== 1 ? "s" : ""} registrada{carreras.length !== 1 ? "s" : ""}
            </p>
          </div>
          <input
        ...
        {loading ? (
            ...
            <div className="empty-state">
              {carreras.length === 0 ? "Sin carreras registradas." : `Sin resultados para "${search}".`}
            </div>
        ) : (
